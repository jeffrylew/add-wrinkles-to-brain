...
//! @brief Get number of items produced after input days with current rates
//! @param[in] machines Reference to vector of machine rates
//! @param[in] days     Number of days machines have to produce items
//! @return Number of items produced in given amount of days
static long num_items_in_days(const std::vector<long>& machines, long days)
{
    // Number of items produced by all machines
    long n_items {};
...
//! @brief Recursive helper to determine the minimum days to achieve goal items
//! @param[in]  machines Reference to vector of machine rates
//! @param[in]  goal     Goal number of items to produce
//! @param[in]  min_days Min number of days to produce goal items
//! @param[in]  max_days Max number of days to produce goal items
//! @param[out] min_time Ref to number of days it takes to produce goal items
static void minTimeRecursiveHelper(const std::vector<long>& machines,
                                   long                     goal,
                                   long                     min_days,
                                   long                     max_days,
                                   long&                    min_time)
{
    const auto mid_days  = (max_days + min_days) / 2L;
    const auto num_items = num_items_in_days(machines, mid_days);
    
    if (min_days == max_days)
    {
        if (num_items == goal)
        {
            // Met goal number of items exactly
            min_time = mid_days;
        }
        else
        {
            min_time = mid_days + 1L;
        }
        return;
    }
    else if (min_days > max_days)
    {
        if (num_items < goal)
        {
            // min_days is not enough and max_days
            // results in more items than goal
            min_time = mid_days + 1L;
        }
        else
        {
    ...
    else if (num_items == goal)
    {
        // min_days < max_days, decrease min_time
        // until goal is not met
        min_time = mid_days - 1L;
        while (goal == num_items_in_days(machines, min_time))
        {
            --min_time;
        }
        ++min_time;
    ...
    {
        minTimeRecursiveHelper(machines,      //
                               goal,          //
                               mid_days + 1L, //
                               max_days,      //
                               min_time);
...
} // static void minTimeRecursiveHelper( ...

//! @brief Attempt to create solution from discussion section. Fails Test case 5
static long minTimeDiscussionSolution(std::vector<long> machines, long goal)
{
    // Minimum number of days to complete the order at current machine rates
    long min_time {};

    const auto n_machines = static_cast<long>(machines.size());

    // Get minimum number of days to complete the order if all machines are fast
    const auto max_rate = *std::min_element(machines.cbegin(), machines.cend());
    const auto min_days = (0 == goal % n_machines)
                        ? goal / n_machines * max_rate
                        : (goal / n_machines + 1L) * max_rate;

    // Get maximum number of days to complete the order if all machines are slow
    const auto min_rate = *std::max_element(machines.cbegin(), machines.cend());
    const auto max_days = (0 == goal % n_machines)
                        ? goal / n_machines * min_rate
                        : (goal / n_machines + 1L) * min_rate;

    // Binary search to find minimum time to complete goal
    minTimeRecursiveHelper(machines, goal, min_days, max_days, min_time);

    return min_time;

} // static long minTimeDiscussionSolution( ...

//! @brief Discussion solution converted from Python
static long minTimeDiscussionSolution2(std::vector<long> machines, long goal)
{
    // Minimum number of days to complete the order at current machine rates
    long min_time {};
    ...
    const auto max_rate = machines.front();
    auto       min_days = (0 == goal % n_machines)
                        ? goal / n_machines * max_rate
                        : (goal / n_machines + 1L) * max_rate;
    
    const auto min_rate = machines.back();
    auto       max_days = (0 == goal % n_machines)
                        ? goal / n_machines * min_rate
                        : (goal / n_machines + 1L) * min_rate;

    while (min_days < max_days)
    {
        const auto mid_day  = (max_days + min_days) / 2L;
        const auto num_items = num_items_in_days(machines, mid_day);
...
TEST(MinTimeTest, TestCase5) {
    ...
    EXPECT_NE(304844592L, minTimeDiscussionSolution(machines, 844676607L));
    EXPECT_EQ(304844592L, minTimeDiscussionSolution2(machines, 844676607L));
}